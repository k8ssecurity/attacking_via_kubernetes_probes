...
> **Caveat — static pods bypass admission control.** Admission policies (`ValidatingAdmissionPolicy`, Kyverno, OPA/Gatekeeper, Pod Security Admission) only see objects that pass through the API server. A **static pod** is created directly by the `kubelet` from a manifest on the node (the `staticPodPath`, usually `/etc/kubernetes/manifests`) and never hits admission — the API server only receives a read-only *mirror pod* of what already ran. So an attacker who can write to a node's manifest directory can run a probe-abusing (or `hostNetwork`/privileged) pod with none of these policies applied, and `hostNetwork` traffic also escapes `NetworkPolicy`. This requires node file access, so treat it as a post-node-compromise concern: protect the kubelet config and manifest path, and detect new static pods at runtime.
